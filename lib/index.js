...
module.exports = function (server, options = {}, routeConfig = {}) {
		...
		readId: {
			options: {
				dataProvider: function () {
					...
					let withRelated = getRelationFromParam(params.params, data, modelInstance, model);
		...
		readOne: {
			options: {
				dataProvider() {
					...
					let withRelated = getRelationFromParam(params, data, query, model);
		...
		readAll: {
			options: {
				dataProvider() {
					...
					let query = collection.query(qb => filter(modelInstance, validateFunc(modelInstance, data), data)(qb));
	...
	function validateFunc(model, attrs, prefix = '') {
		...
		for (let key in attrs) {
			...
			filteredAttributes[key] = item;
		}


		result = joi.validate(filteredAttributes, model.schema || {}, {
			stripUnknown: true,
		});
	...
	function getRelationFromParam(params, data, model, rawModel) {
		let arrayOfRelations = getArrayOfRelationFromParam(params);
		let relations = [];

		for (let key in arrayOfRelations) {
			let relation = arrayOfRelations[key];
			let slpitedRelation = relation.split('.');
			let lastModel = rawModel.forge();
			let queryBuilders = [];

			slpitedRelation.forEach(item => {
					...
					queryBuilders.push(filter(lastModel, validateFunc(lastModel, data, relation), data));
			});

			relations.push({
				[relation](query) {
					queryBuilders.forEach(fn => fn.call(null, query));
				}
			});
		}
