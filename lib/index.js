    ...
    function createRelation(model, data) {
        ...
        return Promise.all(
            relations.map(function (relation) {
                ...
                let related = model.related(name);
                console.log("----------model=",model);
                console.log("----------related=",related);
                console.log("----------related.relation=",related.relation);

                let { relatedData } = related;
                let foreignIdColumn;
                let parentId = relatedData.parentId;

                switch (relatedData.type) {
                  case 'hasOne':
                    foreignIdColumn = relatedData.foreignKeyTarget;
                    break;
                }
                related.set(foreignIdColumn, parentId);
