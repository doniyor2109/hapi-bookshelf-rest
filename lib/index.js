...
function Route(options, routeConfig) {
	...
	internal.getQuery = function () {
		...
		if (internal.filterQuery && typeof internal.filterQuery == 'function') {
			if (query.length) {
				return Promise.map(query, function (item) {
					return Promise
						.resolve(internal.filterPayload.call(internal, item, internal.request))
			...
			return Promise
				.resolve(internal.filterQuery.call(internal, query, internal.request))
				.then(function (filtered) {
	...
	internal.getPayload = function () {
		...
		if (internal.filterPayload && typeof internal.filterPayload == 'function') {
			if (payload.length) {
				return Promise.map(payload, function (item) {
					return Promise
						.resolve(internal.filterPayload.call(internal, item, internal.request))
						.then(function (filtered) {
							return Object.assign(
			...
			return Promise
				.resolve(internal.filterPayload.call(internal, payload, internal.request))
				.then(function (filtered) {
				...
				dataProvider() {
					...
					return this
						...
						.then(function (data) {
							...
							let query = modelInstance.query(qb => filter(collection, validateFunc(modelInstance, data), data)(qb));
	...
	function getRelationFromParam(params, data, model) {
		...
		return Promise.each(arrayOfRelations, (relation, key) => {
			...
			return Promise.each(slpitedRelation, (item) => {
					...
					lastModel.query(qb => {
						filter(lastModel, validateFunc(lastModel, data, item + '.'), data)(qb);
					});

					return lastModel
									.fetch()
									.then(fetchedRelation => {
