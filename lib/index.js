...
module.exports = function (server, options = {}, routeConfig = {}) {
			...
			options: {
				dataProvider: function () {
					...
					return this
						...
						.where(condition)
						.fetch({
							withRelated: getRelationFromParam(params.params, data),
						});
				}
			},
			routeConfig: {
		...
		readOne: {
			options: {
				dataProvider() {
					...
					return query.fetch({
						withRelated: getRelationFromParam(params, data),
					});
				}
			},
			routeConfig: {
		...
		readAll: {
			options: {
				dataProvider() {
					...
					let withRelated = getRelationFromParam(params, data);
	...
	function getRelationFromParam(params, data) {
		let innerRelations = [];
		...
		if (params && params.length) {
			...
			for (let i = 0; i < paramsArray.length; i ++) {
				...
				if (param[0] == '[' && param[paramLength - 1] == ']') {
					innerRelations = innerRelations.concat(
						getRelationFromParam(param.substr(1, paramLength - 1))
					);
				} else if (paramsArray.length - 1 != i) {
					let nextParam = paramsArray[i + 1];

					if (nextParam === 'one') {
						innerRelations.push({
							[param](query) {
								return query.where(getCondition(data, `${param}.`)).first('id');
							}
						});
					} else if ((id = parseInt(nextParam)) > 0) {
						innerRelations.push({
							[param](query) {
								return query.where('id', id).andWhere(getCondition(data, `${param}.`));
							}
						});
					}
					...
					innerRelations.push({
						[param](query) {
							return query.where(getCondition(data, `${param}.`));
						}
					});
				}
			}
